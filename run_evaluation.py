...
def evaluate_model(
    ...
    if algorithm in ["ppo", "sac", "ddpg", "td3", "a2c"]:
        ...
        model = MPCController(
            ...
            reward_mode=reward_mode,
            building_params={"mC": mC, "K": K, "Q_HP_Max": Q_HP_Max, "T_set": 20.2},
            dt_scale=dt_scale,
        )
    else:
...
def main():
    ...
    parser.add_argument(
        "--energy_price_path",
        type=str,
        default="data/langenhagen_price_2025.csv",
    ...
    parser.add_argument(
        "--mC", type=float, default=300, help="Thermal capacitance (J/K)."
    )
    parser.add_argument(
        "--K", type=float, default=20, help="Heat loss coefficient (W/K)."
    )
    parser.add_argument(
        "--Q_HP_Max", type=float, default=1500, help="Max heat pump power (W)."
    )
    parser.add_argument(
        "--cop_heat", type=float, default=1.0, help="Fixed heating COP (ignored if --scop is set)."
    )
    parser.add_argument(
        "--cop_cool", type=float, default=1.0, help="Fixed cooling COP."
    )
    parser.add_argument(
        "--dt-scale",
        type=float,
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Use the IDM AERO ALM 4-12 outdoor-temperature-dependent heating COP curve "
            "(55C flow) instead of the fixed --cop_heat value."
        ),
    )
    parser.add_argument(
        "--prefer_best",
