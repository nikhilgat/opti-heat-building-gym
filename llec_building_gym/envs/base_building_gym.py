...
from llec_building_gym.utils.heat_pump import heating_curve, HeatPump, MAX_PEL_KW
import logging
...
# Module-level singleton: the IDM AERO ALM 4-12 performance model is stateless
# (all state lives in its RegularGridInterpolator objects, built once), so all
# environments/subprocess workers can share one instance.
_HEAT_PUMP = HeatPump()
    ...
    def __init__(
        ...
        Parameters:
            ...
            schedule_type (str): Schedule type for outdoor temperature profile.
                                 ...
                                 Defaults to "24-hour".
            dt_scale (float): Scale factor applied to the thermal update in
                              update_Tin(). The default 1e-3 reproduces the
                              repo's original toy-building calibration
                              (mC=300, K=20). Pass 1.0 when mC/K/Q_HP_Max are
                              true SI values (J/K, W/K, W) so the resulting
                              thermal time constant mC/K (seconds) is
                              physically correct. Defaults to 1e-3.
        """
    ...
    def _create_schedule(self, noise_std=0.1, add_noise=True, seed=None):
        ...
        if self.schedule_type == "simple":
            ...
            if self.outdoor_temperature_path is not None:
                ...
                # Select the temperature column. Single-column files (the
                # original repo format) flatten as before; multi-column
                # files (e.g. a "timestamp, T_out_C" export) use the sole
                # numeric column, so a timestamp column doesn't get mixed
                # into the flattened array.
                numeric_df = outdoor_df.select_dtypes(include=[np.number])
                if numeric_df.shape[1] == 1:
                    outdoor_values = numeric_df.iloc[:, 0].to_numpy()
                elif outdoor_df.shape[1] == 1:
                    outdoor_values = outdoor_df.values.flatten()
                else:
                    raise ValueError(
                        f"Ambiguous outdoor_temperature_path CSV with {outdoor_df.shape[1]} "
                        f"columns ({list(outdoor_df.columns)}) and {numeric_df.shape[1]} numeric "
                        "columns; expected exactly one numeric temperature column."
                    )
...
class BaseBuildingGym(gym.Env):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # If True, heating COP/electrical draw is looked up per-step from the
        # full IDM AERO ALM 4-12 model (llec_building_gym.utils.heat_pump):
        # weather-compensated flow temperature (heating_curve) + 2D
        # (T_air, T_flow) capacity/COP interpolation with duty-cycle
        # averaging below the modulation floor. Replaces the fixed COP_HEAT
        # above. Cooling always uses the fixed COP_COOL (no cooling data on
        ...
        # or the full heat pump model is used. For the default
        # cop_heat=cop_cool=1.0 (toy building), this reduces exactly to
        ...
        self._P_el_worst_case = max(
            heating_worst_case_w, cooling_worst_case_w, self.EPS
        )
    ...
    def step(self, action):
        ...
        # 2.Economic-based reward: negative of energy cost (cost = price × electrical
